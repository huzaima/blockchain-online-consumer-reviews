...
contract ReviewSmartContract{
    ...
    mapping (address => string) IPFSHash; //Store the IPFS Hash Of reviewer
    ...
    function ReviewSmartContract() payable { //Constructor
    ...
    function showReviewNumber() public view returns(uint){
        return ReviewNumber; //shows current balance in the smart contract
    }
    ...
    function IssueToken(address _reviewer) onlySP payable returns(uint256){
    ...
    function reviewDone(address _reviewer,string _ipfshash,uint256 tokenNum) payable{
  ...
  function reward(address _reviewer) payable{
